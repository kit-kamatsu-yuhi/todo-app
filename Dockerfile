...
# CMD から migrate は外れたが、Cloud Run Job（todo-migrate）が同一イメージで
# prisma CLI を実行するため、個別コピーではなく完全な node_modules を同梱する。
...
# migrate は Cloud Run Job（todo-migrate）がデプロイ時に実行する。
# ローカル開発の docker compose は自前の command で migrate を継続する。
CMD ["node", "server.js"]
